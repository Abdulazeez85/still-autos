...
async function loadStats() {
  try {
    ...
    safeSet(
      "stat-value",
      "₦" + (d.inventory_value / 1_000_000_000).toFixed(1) + "B",
    );
    ...
    if (container && d.recent_inquiries?.length) {
      container.innerHTML = d.recent_inquiries
        .map(
          (i) => `
        <tr>
...
function renderVehiclesTable(list) {
  ...
  if (!list.length) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align:center;color:var(--text-muted);padding:2rem">No vehicles found</td></tr>';
    return;
  }
...
function searchVehicles() {
  const q = document
    .getElementById("vehicles-search")
    ?.value?.toLowerCase()
    ?.trim();
...
function openEditModal(id) {
  ...
  if (imgList) {
    imgList.innerHTML = (v.images || [])
      .map((url, i) => imageRow(url, i))
      .join("");
  }
...
async function saveVehicle(e) {
  ...
  try {
    ...
    const data = {
      ...
      horsepower:
        parseInt(document.getElementById("vf-horsepower")?.value) || 0,
    ...
    if (res.success) {
      showToast(
        id ? "Vehicle updated successfully" : "Vehicle added successfully",
        "success",
      );
...
async function toggleAvailability(id, currentStatus) {
  try {
    const res = await adminAPI("PUT", `/vehicles/${id}`, {
      available: !currentStatus,
    });
    if (res.success) {
      showToast(
        `Vehicle marked as ${!currentStatus ? "available" : "sold"}`,
        "success",
      );
      await loadVehicles();
    }
