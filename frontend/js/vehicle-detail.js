...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!id) { window.location.href = '/inventory'; return; }
...
function renderVehicle(v) {
  ...
  const mileageDisplay = v.mileage === 0 ? 'Brand New' : `${v.mileage.toLocaleString()} km`;
  ...
  if (featuresGrid && v.features) {
    featuresGrid.innerHTML = v.features.map(f => `
  ...
  if (specsTable) {
    const specs = [
      ...
      ['Body Type', v.body], ['Condition', v.condition === 'new' ? 'Brand New' : 'Pre-Owned'],
      ['Engine', v.engine], ['Horsepower', `${v.horsepower} hp`],
      ['Torque', v.torque], ['0-100 km/h', v.acceleration],
      ['Transmission', v.transmission], ['Drive', v.drive],
      ['Fuel Type', v.fuel], ['Colour (Ext.)', v.color_ext],
      ['Colour (Int.)', v.color_int], ['Seats', v.seats],
      ['Mileage', mileageDisplay], ['Location', v.location],
      ['VIN', v.vin]
    ];
    specsTable.innerHTML = specs.map(([k, val]) => `
  ...
  // WhatsApp buttons
  const waMsg = encodeURIComponent(`Hi Still Autos, I'm interested in the *${v.year} ${v.make} ${v.model}* listed at *${priceM}*. Please send more details and arrange a viewing.`);
  const waUrl = `https://wa.me/2348012345678?text=${waMsg}`;
...
function renderGallery(images) {
  ...
  document.getElementById('gallery-main')?.addEventListener('click', () => openLightbox(currentImageIndex));
}
...
function prevImage() { setImage((currentImageIndex - 1 + (currentVehicle?.images?.length || 1)) % (currentVehicle?.images?.length || 1)); }
function nextImage() { setImage((currentImageIndex + 1) % (currentVehicle?.images?.length || 1)); }
...
async function updateMiniCalc() {
  const price = parseFloat(document.getElementById('mini-calc-vehicle-price')?.value || 0);
  const deposit = parseFloat(document.getElementById('mini-deposit')?.value || 20);
  ...
  try {
    ...
    if (res.success) {
      ...
      if (el) el.textContent = SA.Currency.formatMillion(res.data.monthly_payment);
      const depositEl = document.getElementById('mini-calc-deposit');
      if (depositEl) depositEl.textContent = SA.Currency.formatMillion(res.data.deposit_amount);
    }
  } catch (e) {}
}
...
function initInquiryForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    btn.innerHTML = '<span class="loading-spinner" style="width:16px;height:16px;border-width:2px"></span> Sending...';
    btn.disabled = true;
    ...
    try {
      ...
      SA.Toast.show('Your enquiry has been sent! We\'ll contact you shortly.', 'success');
      form.reset();
    } catch (err) {
      SA.Toast.show('Failed to send enquiry. Please try WhatsApp instead.', 'error');
    } finally {
